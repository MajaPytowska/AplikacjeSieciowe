...
class CalcCtrl {
    ...
    private function calculate(){
        ...
        if($this->form->procent < 5 && !inRole('admin')){
           getMessages()->addError('Tylko administrator może wziąć kredyt z oprocentowaniem poniżej 5%.');
        }
        
        if(getMessages()->isError()) return; //jeśli są błędy to przerwij obliczenia
    ...
    private function saveToDB(){
        if($this->result->result_value == null) return; //jeśli brak wyniku to przerwij zapis
        try{
                getDB()->insert('calculation', [ //zapis do bazy danych
                'loan' => $this->form->credit,
                'years' => $this->form->years,
                'procent' => $this->form->procent,
                'result' => $this->result->result_value,
                'date' => date('Y-m-d H:i:s')
            ]);
            } catch (PDOException $e){
				getMessages()->addError('Wystąpił nieoczekiwany błąd podczas zapisu rekordu'. $e->getMessage());			
			}
    }
    ...
    public function generateView(){
        ...
        //ustawienie powiązanej nawigacji
        if(inRole('admin'))
            getNavConfig()->set('Historia obliczeń','showCalcTable');
        else
            getNavConfig()->set(null,null);
